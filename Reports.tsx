...
const Reports: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-24 md:pb-20 animate-premium">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          <button onClick={() => setShowMasterReport(true)} className="bg-indigo-600 text-white px-5 py-4 rounded-2xl font-black text-xs shadow-xl shadow-indigo-600/20">MASTER REPORT</button>
          
          <div className="grid grid-cols-2 gap-3 w-full sm:w-auto">
            <select className="px-4 py-4 rounded-2xl border bg-white font-bold outline-none" value={selectedMonth} onChange={e => setSelectedMonth(Number(e.target.value))}>
              {months.map((m, i) => <option key={m} value={i}>{m}</option>)}
            </select>
            <select className="px-4 py-4 rounded-2xl border bg-white font-bold outline-none" value={selectedYear} onChange={e => setSelectedYear(Number(e.target.value))}>
