...
// Standard react-router-dom v6 hooks and components
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 animate-premium">
      <div className="w-full max-w-md">
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl font-black text-slate-800 tracking-tighter uppercase">{settings?.canteenName || 'Elite Login'}</h1>
        </div>

        <div className="flex bg-slate-200/50 p-1.5 rounded-[32px] border border-slate-200 mb-8">
          <button onClick={() => setLoginType('customer')} className={`flex-1 py-4 rounded-[28px] font-black text-[10px] uppercase tracking-widest transition-all ${loginType === 'customer' ? 'bg-white text-indigo-600 shadow-xl' : 'text-slate-500'}`}>Member</button>
          <button onClick={() => setLoginType('admin')} className={`flex-1 py-4 rounded-[28px] font-black text-[10px] uppercase tracking-widest transition-all ${loginType === 'admin' ? 'bg-white text-indigo-600 shadow-xl' : 'text-slate-500'}`}>Manager</button>
        </div>
        ...
        <form onSubmit={handleLogin} className="bg-white p-12 rounded-[56px] border border-slate-100 shadow-2xl space-y-8">
          <div className="space-y-3">
            ...
            <input 
              ...
              required
              className="w-full px-8 py-6 rounded-[32px] bg-slate-900 text-white font-black text-lg focus:ring-4 ring-indigo-500/20 outline-none transition-all"
          ...
          <button type="submit" className="w-full bg-indigo-600 text-white py-7 rounded-[32px] font-black text-xl shadow-xl hover:bg-indigo-700 active:scale-95 transition-all">ESTABLISH SESSION</button>
        </form>

        <div className="mt-12 text-center">
          <Link to="/" className="text-slate-400 hover:text-indigo-600 font-black text-[10px] uppercase tracking-[0.4em]">← Dashboard</Link>
        </div>
      </div>
    </div>
  );
