...
import { TrendingUp, Search, X, Utensils, ChefHat, Timer, Check, Plus, AlertCircle, Bell, CheckCircle, XCircle, Loader2, Sparkles, Image as ImageIcon } from 'lucide-react';
...
const Dashboard: React.FC<{ role: string }> = ({ role }) => {
  ...
  const handleCancelDemand = async (id: number) => {
    ...
    try {
      const { error } = await supabase.from('demands').update({ status: 'cancelled' }).eq('id', id);
    ...
    <div className="space-y-8 animate-premium pb-12">
      ...
      <section className="bg-white p-8 lg:p-10 rounded-[40px] border border-slate-100 shadow-xl">
        <div className="flex items-center justify-between mb-8">
          <h3 className="text-xl lg:text-2xl font-black text-slate-800 flex items-center gap-3 uppercase">
            <Sparkles className="text-amber-500 fill-amber-500" /> Today's Selection
          </h3>
          <Link to="/inventory" className="text-xs font-black text-indigo-600 uppercase tracking-widest hover:underline decoration-2">Full Menu →</Link>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {data.inventory.filter(i => data.dailyMenuIds.includes(i.id!)).map(item => (
            <div key={item.id} className="p-6 rounded-[32px] bg-slate-50 border border-slate-100 flex flex-col justify-between hover:bg-white hover:border-amber-200 hover:shadow-xl transition-all duration-300 relative overflow-hidden group">
              <div>
                <div className="flex items-start gap-4 mb-4">
                  <div className="w-14 h-14 lg:w-16 lg:h-16 shrink-0 rounded-2xl bg-white border border-slate-100 shadow-inner overflow-hidden flex items-center justify-center text-slate-300">
                    {item.image_url ? (
                  ...
                  <div className="min-w-0 flex-1 pt-1">
                    <div className="text-[9px] font-black text-amber-600 uppercase mb-1 tracking-widest">Selection</div>
                    <div className="font-black text-slate-800 text-base lg:text-lg tracking-tight leading-tight uppercase line-clamp-2">{item.item_name}</div>
                  </div>
                </div>
                <div className="mt-1 text-[8px] font-bold text-slate-400 uppercase tracking-widest italic flex items-center gap-1">
                  <AlertCircle size={8} /> Rates not fixed
                </div>
              </div>
              <div className="mt-6 flex items-center justify-between">
                <span className={`px-3 py-1 rounded-lg text-[8px] font-black border ${item.stock_quantity > 0 ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100'}`}>
                  {item.stock_quantity > 0 ? 'INSTOCK' : 'SOLD OUT'}
                </span>
                {role === 'customer' && (
                  <Link to="/my-statement" className="p-2 bg-slate-900 text-white rounded-lg hover:bg-indigo-600 transition-colors">
                    <Plus size={14} />
                  </Link>
                )}
              </div>
          ...
          {data.dailyMenuIds.length === 0 && (
            <div className="col-span-full py-12 text-center bg-slate-50 rounded-[32px] border-2 border-dashed border-slate-200">
              <Utensils className="mx-auto mb-3 text-slate-300" size={32} />
              <p className="text-slate-400 font-bold italic text-sm">No special items curated for today.</p>
            </div>
          )}
        </div>
      </section>
        ...
        <div className="space-y-8">
          <div className="bg-slate-900 p-10 lg:p-12 rounded-[40px] text-white shadow-2xl relative overflow-hidden">
            ...
            <input 
              ...
              className="w-full bg-slate-800 border border-slate-700 rounded-2xl p-5 text-white font-black mb-4 focus:ring-4 ring-indigo-500/20 outline-none text-center tracking-widest uppercase"
            ...
            <button 
              ...
              className="w-full bg-indigo-600 py-5 rounded-2xl font-black text-[10px] tracking-widest uppercase hover:bg-indigo-50 transition-all"
            >
              VALIDATE IDENTITY
            </button>
          </div>
      ...
      {lookupResult && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md">
          <div className="bg-white w-full max-w-md rounded-[48px] shadow-2xl p-10 text-center relative">
            ...
            <div className="w-24 h-24 bg-indigo-50 text-indigo-600 rounded-[32px] flex items-center justify-center mx-auto mb-8 shadow-xl border border-indigo-100 font-black">{lookupResult.name.charAt(0)}</div>
            <h3 className="text-2xl font-black text-slate-800 uppercase">{lookupResult.name}</h3>
            ...
            <button onClick={() => setLookupResult(null)} className="w-full bg-indigo-600 text-white py-6 rounded-[28px] font-black tracking-widest uppercase text-xs shadow-xl active:scale-95">CLOSE VIEW</button>
          </div>
        </div>
      )}
              ...
              {filteredCurationMenu.map(item => (
                ...
                >
                  <div className="text-left flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-white border overflow-hidden flex items-center justify-center text-slate-300">
                      {item.image_url ? <img src={item.image_url} className="w-full h-full object-cover" /> : <Utensils size={16} />}
                    </div>
                    <div>
                      <span className="font-black text-lg block leading-tight">{item.item_name}</span>
                      {isAdmin && <span className="text-xs font-bold opacity-60">৳{item.price}</span>}
                    </div>
                  </div>
