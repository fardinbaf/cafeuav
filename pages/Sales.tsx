...
import { ShoppingCart, Plus, Minus, X, CheckCircle2, Search, Calculator, Shield, Car, PlusCircle, Scan, ArrowDown, XCircle } from 'lucide-react';
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 min-h-screen lg:h-[calc(100vh-140px)] animate-premium">
      <div className="lg:col-span-7 flex flex-col gap-8 overflow-hidden">
        <div className="flex justify-between items-center px-2 lg:px-0">
          <div>
            <h2 className="text-3xl font-black text-slate-800 tracking-tight uppercase leading-none">Canteen POS</h2>
            <div className="flex items-center gap-2 mt-1">
              <Scan size={14} className="text-indigo-600" />
              <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest">Active Terminal</p>
            </div>
          </div>
          <button onClick={() => { setCustomItem({ name: '', price: '' }); setShowCustomModal(true); }} className="flex items-center gap-2 bg-slate-900 text-white px-6 py-4 rounded-[24px] font-black text-[10px] uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-xl">
      ...
      <div className="lg:col-span-5 bg-white rounded-[56px] border border-slate-200 shadow-2xl flex flex-col overflow-hidden border-t-8 border-t-indigo-600 relative mx-2 lg:mx-0">
        <div className="p-8 lg:p-10 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
          <h3 className="font-black flex items-center gap-3 text-slate-800 uppercase tracking-tight text-lg">
            <ShoppingCart size={24} className="text-indigo-600" /> Basket
          </h3>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-4 custom-scrollbar min-h-[200px]">
          {cart.map((item, idx) => (
            <div key={`${item.item_name}-${item.price}-${idx}`} className="flex items-center justify-between p-5 bg-slate-50 rounded-[32px] border border-slate-100 hover:bg-white transition-all">
          ...
          {cart.length === 0 && <div className="h-full flex flex-col items-center justify-center text-slate-300 gap-4 py-20 opacity-30 uppercase font-bold tracking-widest italic">Basket Empty</div>}
        </div>

        <div className="p-8 lg:p-10 bg-slate-50 border-t border-slate-200 space-y-6 shadow-[0_-20px_40px_rgba(0,0,0,0.02)]">
          <div className="space-y-4">
            <div className="relative" ref={dropdownRef}>
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-2 block ml-1">Member Identification</label>
          ...
          <div className="pt-6 border-t border-slate-200">
            ...
            {lastTransaction && (
              <button onClick={() => setShowReceipt(true)} className="w-full mt-4 py-3 text-indigo-600 font-black text-[10px] uppercase tracking-widest underline underline-offset-8 decoration-indigo-200 transition-all flex items-center justify-center gap-2">
                <ArrowDown size={14} /> Memo #{lastTransaction.id}
              </button>
            )}
          </div>
        </div>
      ...
      {showCustomModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-xl animate-fade-in">
          <div className="bg-white w-full max-w-md rounded-[56px] p-10 lg:p-12 space-y-8 shadow-2xl border border-white/20">
            <h3 className="text-2xl font-black tracking-tighter uppercase text-slate-800">Custom Charge</h3>
            <div className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Label</label>
                <input className="w-full p-6 bg-slate-900 text-white rounded-[28px] font-black text-sm uppercase outline-none" placeholder="Description" value={customItem.name} onChange={e => setCustomItem({...customItem, name: e.target.value})} />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Rate (৳)</label>
                <input autoFocus type="number" className="w-full p-6 bg-slate-900 text-white rounded-[28px] font-black text-3xl outline-none text-center" placeholder="0.00" value={customItem.price} onChange={e => setCustomItem({...customItem, price: e.target.value})} />
              </div>
            </div>
            <div className="flex flex-col gap-2">
              <button onClick={() => { if(!customItem.name || !customItem.price) return; addToCart({id: Date.now(), item_name: customItem.name, price: Number(customItem.price)}); setShowCustomModal(false); }} className="w-full bg-indigo-600 text-white py-6 rounded-[28px] font-black text-lg shadow-xl active:scale-95 transition-all uppercase">INJECT TO CART</button>
              <button onClick={() => setShowCustomModal(false)} className="w-full py-4 text-slate-400 font-bold uppercase text-[10px] tracking-widest">DISCARD</button>
            </div>
          </div>
        </div>
      )}
