...
import { ShoppingCart, Utensils, Zap, Search, User, CreditCard, Loader2, AlertCircle, History, XCircle, Timer } from 'lucide-react';
...
const MyStatement: React.FC<MyStatementProps> = ({ uid }) => {
  ...
  const handlePlaceDemand = async (item: InventoryItem) => {
    ...
    if (!isOrderingOpen) {
      alert("Terminal Closed: Service resumes at 08:00 PM tonight.");
      return;
    }
  ...
  const handleCancelDemand = async (id: number) => {
    if (!confirm("Revoke request?")) return;
  ...
  if (loading) return <div className="h-[80vh] flex flex-col items-center justify-center gap-4 animate-premium"><Loader2 className="animate-spin text-indigo-600" size={48} /><p className="font-black text-xs uppercase tracking-widest text-slate-400">Pulling Secure Ledger...</p></div>;
    ...
    <div className="max-w-5xl mx-auto space-y-10 pb-12 animate-premium">
      <div className="flex flex-col items-center text-center space-y-6 pt-10">
          <div className="relative">
            <div className="w-24 h-24 lg:w-32 lg:h-32 bg-slate-900 text-white rounded-[40px] flex items-center justify-center shadow-2xl ring-8 ring-indigo-50">
              <User size={56} />
            </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-center gap-2">
              <span className="bg-indigo-600 text-white px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">PERSONAL PORTAL</span>
              <span className="text-slate-400 font-bold text-xs uppercase tracking-widest">ID #{customer?.uid}</span>
            </div>
            <h2 className="text-4xl lg:text-6xl font-black text-slate-800 tracking-tighter leading-none uppercase">
              {customer?.name}
            </h2>
          </div>

          <div className="bg-slate-900 px-12 py-10 rounded-[56px] shadow-[0_40px_80px_-20px_rgba(0,0,0,0.4)] border-b-8 border-indigo-600 min-w-[320px] animate-premium transform hover:scale-105 transition-transform duration-500">
            <div className="text-[11px] font-black text-indigo-400 uppercase tracking-[0.4em] mb-4">Liability Assessment</div>
            <div className="text-6xl lg:text-7xl font-black text-white tracking-tighter mb-2">৳{customer?.total_baki.toLocaleString()}</div>
            <p className="text-[10px] text-indigo-400/50 font-bold uppercase tracking-widest italic">Live Accounting Balance</p>
          </div>
      </div>
      ...
      <section className="bg-white p-8 lg:p-10 rounded-[48px] border shadow-xl border-slate-100">
        <div className="flex flex-col sm:flex-row justify-between items-center mb-10 gap-6">
          <div className="w-full">
            <h3 className="text-2xl font-black text-slate-800 flex items-center gap-3 uppercase"><Utensils className="text-indigo-600" /> Active Service</h3>
            <div className="flex items-center gap-3 mt-2">
              <div className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-[10px] font-black uppercase ${isOrderingOpen ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                <Timer size={14} className={isOrderingOpen ? "animate-pulse" : ""} /> {isOrderingOpen ? 'Ordering Active' : 'Station Closed'}
              </div>
              <span className="text-slate-400 font-bold text-[10px] uppercase tracking-widest">20:00 PM - 12:00 PM</span>
            </div>
          </div>
        ...
        {!isOrderingOpen && (
          <div className="mb-8 p-6 bg-rose-50 border border-rose-100 rounded-[32px] flex items-center gap-4 text-rose-600">
            <AlertCircle size={32} /><p className="font-black text-xs uppercase">The pre-ordering terminal is currently offline. Ordering will resume automatically at 08:00 PM.</p>
          </div>
        )}
          ...
          {dailyMenuItems.map(item => (
            <div key={item.id} className="bg-slate-50 p-6 lg:p-8 rounded-[40px] border border-slate-100 flex justify-between items-center transition-all hover:shadow-2xl hover:bg-white group">
              <div>
                ...
                <div className="mt-1 text-[8px] font-bold text-slate-400 uppercase tracking-widest italic flex items-center gap-1">
                  <AlertCircle size={8} /> Rate not fixed
                </div>
              </div>
              <button 
      ...
      <section className="bg-white rounded-[48px] border shadow-2xl border-slate-100 overflow-hidden">
        <div className="p-8 border-b bg-slate-50 flex items-center justify-between">
          <h3 className="font-black text-slate-800 uppercase tracking-tight flex items-center gap-3"><History size={22} className="text-indigo-600" /> Activity Log</h3>
          <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Recent Entries</div>
        </div>
          ...
          {myDemands.filter(d => d.status === 'pending').map(d => (
            <div key={d.id} className="p-8 bg-indigo-50/50 flex items-center justify-between animate-pulse">
               <div className="flex gap-6 items-center">
                 ...
                 <div><div className="font-black text-xl text-indigo-900 uppercase">PRE-ORDER: {d.item_name}</div><div className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest mt-1">Status: Pending Kitchen</div></div>
               </div>
          ...
          {myHistory.map((t) => (
            <div key={t.id} className="p-8 flex items-center justify-between hover:bg-slate-50 transition-colors">
              <div className="flex gap-6 items-center">
                ...
                <div><div className="font-black text-xl text-slate-800 tracking-tight uppercase">{t.type === 'payment' ? 'Hishab Settlement' : (t.items || []).map(i => i.item_name).join(', ')}</div><div className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-1">{new Date(t.timestamp).toLocaleDateString()} • {t.payment_type}</div></div>
              </div>
