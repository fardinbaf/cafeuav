...
// Standard react-router-dom v6 named exports
import { Link, useLocation } from 'react-router-dom';
import { useLiveQuery } from 'dexie-react-hooks';
import { db } from '../db';
import { 
  LayoutDashboard, Users, Package, ShoppingCart, BarChart3, Settings as SettingsIcon,
  Menu, X, Wifi, WifiOff, LogOut, LogIn, FileText, User, Receipt, HelpCircle
} from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ isOnline, role, onLogout }) => {
  ...
  const adminMenu = [
    ...
    { name: 'Member DB', icon: Users, path: '/customers' },
    { name: 'Inventory', icon: Package, path: '/inventory' },
    { name: 'Expenditures', icon: Receipt, path: '/expenses' },
    { name: 'Reports', icon: BarChart3, path: '/reports' },
    { name: 'Settings', icon: SettingsIcon, path: '/settings' },
    { name: 'Guide', icon: HelpCircle, path: '/help' },
  ];
  ...
  const customerMenu = [
    { name: 'Home', icon: LayoutDashboard, path: '/' },
    { name: 'Personal Portal', icon: FileText, path: '/my-statement' },
    { name: 'Inventory', icon: Package, path: '/inventory' },
    { name: 'Help', icon: HelpCircle, path: '/help' },
  ];
  ...
  return (
    <>
      ...
      `}>
        <div className="p-8">
          <Link to="/" onClick={() => setIsOpen(false)} className="flex items-center gap-3 mb-2 hover:opacity-80 transition-opacity">
            ...
            <h1 className="text-lg font-black text-slate-800 leading-tight tracking-tighter uppercase">
              {settings?.canteenName || 'Cafe UAV'}
            </h1>
          </Link>
          <div className="flex items-center gap-2 px-3 py-1 bg-slate-100 rounded-full w-fit">
            {isOnline ? (
              <><Wifi size={10} className="text-green-500" /> <span className="text-[8px] text-slate-500 font-black uppercase tracking-wider">Connected</span></>
            ) : (
              <><WifiOff size={10} className="text-rose-500" /> <span className="text-[8px] text-slate-500 font-black uppercase tracking-wider">Local Only</span></>
            )}
          </div>
        </div>

        <nav className="mt-2 px-4 space-y-1 overflow-y-auto max-h-[calc(100vh-280px)] custom-scrollbar">
          {menuItems.map((item) => {
        ...
        <div className="absolute bottom-8 left-0 w-full px-6 space-y-4">
          {role === 'guest' ? (
            <Link to="/login" className="w-full flex items-center justify-center gap-2 py-4 bg-indigo-50 text-indigo-600 rounded-2xl font-black text-xs hover:bg-indigo-100 transition-all border border-indigo-100 uppercase tracking-widest">
              <LogIn size={16} /> LOGIN
            </Link>
          ) : (
            <button onClick={onLogout} className="w-full flex items-center justify-center gap-2 py-4 bg-rose-50 text-rose-600 rounded-2xl font-black text-xs hover:bg-rose-100 transition-all border border-rose-100 uppercase tracking-widest">
          ...
          <div className="p-4 bg-slate-900 rounded-[28px] text-white shadow-2xl flex items-center gap-3 border border-white/5">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs font-black truncate">{settings?.managerName || 'Running Manager'}</p>
            </div>
          </div>
        </div>
      </div>
