...
import { X, Printer, Share2, Calculator } from 'lucide-react';
...
const StatementModal: React.FC<StatementModalProps> = ({ customer, onClose }) => {
    ...
    const fetchStatementData = async () => {
      ...
      try {
        const [
          ...
          supabase.from('transactions').select('*').eq('customer_id', customer.id)
        ]);
  ...
  if (loading) return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm">
      <div className="bg-white p-10 rounded-[40px] shadow-2xl animate-pulse font-black text-slate-400">BUILDING STATEMENT...</div>
    </div>
  );

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 sm:p-4 bg-slate-900/60 backdrop-blur-md">
      <div className="bg-white w-full max-w-5xl rounded-[32px] md:rounded-[40px] shadow-2xl overflow-hidden flex flex-col h-[95vh] animate-in zoom-in duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-12 bg-white custom-scrollbar printable-area">
          <div className="max-w-2xl mx-auto font-serif text-black bg-white">
            <table className="w-full border-2 border-black border-collapse text-sm md:text-lg">
              <tbody>
                ...
                <tr>
                  ...
                  <td className="border-2 border-black p-2 md:p-3 text-center font-bold">{displayName} ({customer.uid})</td>
                </tr>
              </tbody>
            </table>
              ...
              <tbody>
                ...
                {previousArrears !== 0 && (
                  <tr className="opacity-70">
                    <td colSpan={3} className="border-2 border-black p-2 text-center font-bold italic text-sm md:text-base">পূর্বের বকেয়া/ব্যালেন্স</td>
            ...
            <div className="mt-8 md:mt-12 flex justify-between px-4 md:px-10">
              <div className="text-center pt-8 md:pt-10 border-t border-black w-32 md:w-40 text-xs md:text-base">গ্রাহকের স্বাক্ষর</div>
              <div className="text-center pt-8 md:pt-10 border-t border-black w-32 md:w-40 font-bold text-xs md:text-base">ম্যানেজার</div>
            </div>
          </div>
        </div>
